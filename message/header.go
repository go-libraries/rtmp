...
func ReadBaseHeader(buf Reader) (n int, fmt uint8, csi uint32, err error) {
	var b byte
	b, err = ReadByteFromNetwork(buf)
	if err != nil {
		return
	}
	n = 1
...
func (header *Header) ReadHeader(buf Reader, vfmt uint8, csi uint32, lastheader *Header) (n int, err error) {
	header.Fmt = vfmt
	...
	case HeaderFmtFull:
		...
		_, err = ReadAtLeastFromNetwork(buf, tmpBuf[1:], 3)
		if err != nil {
			return
		}
		n += 3
		...
		_, err = ReadAtLeastFromNetwork(buf, tmpBuf, 4)
		if err != nil {
			return
		}
		n += 4
		header.MessageStreamID = binary.LittleEndian.Uint32(tmpBuf)
	case HeaderFmtSameStream:
	...
	case HeaderFmtSameLengthAbdStream:
	...
	// Todo: Test with FMS
	if (header.Fmt != HeaderFmtContinuation && header.Timestamp >= 0xffffff) ||
		(header.Fmt == HeaderFmtContinuation && lastheader != nil && lastheader.ExtendedTimestamp > 0) {
		_, err = ReadAtLeastFromNetwork(buf, tmpBuf, 4)
		if err != nil {
			return
		}
...
func (header *Header) Write(buf Writer) (n int, err error) {
	// Write fmt & Chunk stream ID
	switch {
	case header.ChunkStreamID <= 63:
		err = buf.WriteByte(byte((header.Fmt << 6) | byte(header.ChunkStreamID)))
		if err != nil {
			return
		}
		n += 1
	case header.ChunkStreamID <= 319:
		err = buf.WriteByte(header.Fmt << 6)
		if err != nil {
			return
		}
		n += 1
		err = buf.WriteByte(byte(header.ChunkStreamID - 64))
		if err != nil {
			return
		}
		n += 1
	case header.ChunkStreamID <= 65599:
		err = buf.WriteByte((header.Fmt << 6) | 0x01)
		if err != nil {
			return
		}
		n += 1
	...
	case HeaderFmtFull:
		...
		n += m
		// Message Type
		err = buf.WriteByte(header.MessageTypeID)
		if err != nil {
			return
		}
		n += 1
		// Message Stream ID
		err = binary.Write(buf, binary.LittleEndian, &(header.MessageStreamID))
		if err != nil {
			return
		}
		n += 4
	...
	if header.Timestamp >= 0xffffff {
		// Extended Timestamp
		err = binary.Write(buf, binary.BigEndian, &(header.ExtendedTimestamp))
		if err != nil {
			return
		}
		n += 4
